...
//	private static final int errorColor = Color.BLACK;
	...
	public void resetValidators(Context context) {
		...
		case TEST_ALPHANUMERIC:
			toAdd = new AlphaNumericValidator(
					TextUtils.isEmpty(testErrorString) ? context.getString(R.string.error_this_field_cannot_contain_special_character) : testErrorString);
			break;

		case TEST_NUMERIC:
			toAdd = new NumericValidator(TextUtils.isEmpty(testErrorString) ? context.getString(R.string.error_only_numeric_digits_allowed) : testErrorString);
			break;
		case TEST_NUMERIC_RANGE:
			toAdd = new NumericRangeValidator(TextUtils.isEmpty(testErrorString) ? context.getString(R.string.error_only_numeric_digits_range_allowed,
					minNumber, maxNumber) : testErrorString, minNumber, maxNumber);
			break;
		case TEST_REGEXP:
		...
		case TEST_CREDITCARD:
			toAdd = new CreditCardValidator(TextUtils.isEmpty(testErrorString) ? context.getString(R.string.error_creditcard_number_not_valid)
					: testErrorString);
			break;
		case TEST_EMAIL:
			toAdd = new EmailValidator(TextUtils.isEmpty(testErrorString) ? context.getString(R.string.error_email_address_not_valid) : testErrorString);
			break;
		case TEST_PHONE:
		...
		case TEST_DOMAINNAME:
			toAdd = new DomainValidator(TextUtils.isEmpty(testErrorString) ? context.getString(R.string.error_domain_not_valid) : testErrorString);
			break;
		case TEST_IPADDRESS:
			toAdd = new IpAddressValidator(TextUtils.isEmpty(testErrorString) ? context.getString(R.string.error_ip_not_valid) : testErrorString);
			break;
		case TEST_WEBURL:
		...
		case TEST_PERSONNAME:
			toAdd = new PersonNameValidator(TextUtils.isEmpty(testErrorString) ? context.getString(R.string.error_notvalid_personname) : testErrorString);
			break;
		case TEST_PERSONFULLNAME:
			toAdd = new PersonFullNameValidator(TextUtils.isEmpty(testErrorString) ? context.getString(R.string.error_notvalid_personfullname)
					: testErrorString);
			break;
		...
		case TEST_CUSTOM:
			...
			try {
				...
				throw new RuntimeException(String.format("Unable to construct custom validator (%s) with argument: %s", classType, testErrorString));
			}
	...
	public void showUIError() {
		if (mValidator.hasErrorMessage()) {
		    ...
		    else{ // for editText that open fragment to pick date then display it
		    	editText.setFocusableInTouchMode(true);
		    	editText.requestFocus();
		    	editText.setError(ssbuilder);
		    	editText.setFocusableInTouchMode(false);
		    }
//			editText.setError(Html.fromHtml("<font color='" + BLACK + "'>" + mValidator.getErrorMessage() + "</font>"));
		}
	}
